<template>
  <el-dialog :title="title" v-model="dialogFormVisible" customClass="dlg-classify-list">
    <div class="list_content" v-loading="loading">
      <ul v-if="type === 'class'">
        <li v-for="item in classify" @click="btnClass(item)"><span>{{item.code}}</span><span>{{item.type}}</span>{{item.name}}{{item.desc}}</li>
      </ul>
        ...
        <el-table :data="groupFun" style="width: 100%" :show-header="false"
          ...
          <el-table-column type="expand">
            <template scope="props">
              <el-form>
                <el-form-item>
                  <p class="groupChild" v-for="child in props.row.child"><span>{{ child.code }}</span><span>{{ child.name }}</span></p>
                </el-form-item>
              </el-form>
            </template>
          </el-table-column>
